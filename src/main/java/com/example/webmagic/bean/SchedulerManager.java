package com.example.webmagic.bean;
...
public class SchedulerManager {
	...
	public List<JobDetail> listAllJobs() {
		...
		try {
			...
			for (String group : groupList) {
				Set<JobKey> jobKes = scheduler.getJobKeys(GroupMatcher.jobGroupEquals(group));
	...
	public void addJob(JobBean jobBean) {
		try {
			...
			if (!scheduler.checkExists(jobKey)) {
				JobDetail detail = JobBuilder.newJob(clazz).withDescription(jobBean.getDesc()).withIdentity(jobKey)
						.build();// 任务名，任务组，任务执行类
				// 触发器1.构造时间表达式
				CronScheduleBuilder builder = CronScheduleBuilder.cronSchedule(jobBean.getCronExpression());// 触发器时间设定
				// 构造触发器
				CronTrigger trigger = (CronTrigger) TriggerBuilder.newTrigger()
						.withIdentity(jobBean.getJobName(), jobBean.getJobGroup()).withSchedule(builder).build();
	...
	public void modifyJobTime(JobBean jobBean) {
		try {
			// 取得该触发器
			TriggerKey triggerKey = new TriggerKey(jobBean.getJobName(), jobBean.getJobGroup());
			...
			if (!oldCronExpression.equalsIgnoreCase(jobBean.getCronExpression())) {
				CronScheduleBuilder builder = CronScheduleBuilder.cronSchedule(jobBean.getCronExpression());// 触发器时间设定
				// 立刻重新启动触发器
				CronTrigger newTrigger = trigger.getTriggerBuilder().withIdentity(triggerKey).withSchedule(builder)
						.build();
				scheduler.rescheduleJob(triggerKey, newTrigger);
			}else{
				Log.info("修改未执行"+(!oldCronExpression.equalsIgnoreCase(jobBean.getCronExpression())));
			}
		} catch (Exception e) {
	...
	public String getTriggerState(TriggerKey triggerKey) {
		try {
			TriggerState triggerState;// NONE, NORMAL, PAUSED, COMPLETE, ERROR,
										// BLOCKED
